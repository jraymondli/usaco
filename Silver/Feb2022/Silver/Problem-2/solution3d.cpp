...
int rstep[40][2];
long long result[41];
map<int, map<int, map<int, long long> > > fmoves;
map<int, map<int, map<int, long long> > > rmoves;

queue<tuple<int, int, int, long long> > updates;

void traverse(map<int, map<int, map<int, long long> > > &moves, int start_x, int start_y, int end_x, int end_y, int N, int step[][2]) 
{
...
int main() 
{
   ...
   if ( N <= 20) {
     traverse(fmoves, 0, 0, xg, yg, N, fstep);
   } else {
	int N1 = N /2;
        int N2 = N - N1;
    
	traverse(fmoves, 0, 0, xg, yg, N1, fstep); 

	for (int i = 0; i < N2; i ++) {
	    rstep[i][0] = - fstep[N-i-1][0];
	    rstep[i][1] = - fstep[N-i-1][1];
 	}
	traverse(rmoves, xg, yg, 0, 0, N2, rstep);
